...
The program loads recipes from `receipts.txt` and presents an interactive menu with arrow key navigation:
- Display all recipes
- Add new recipes
...
**Navigation:**
- Use **UP/DOWN arrow keys** to navigate through menu options
...
The application uses raw terminal mode to provide an interactive menu experience (`main.c:130-153`):
...
This allows:
- Real-time arrow key detection without pressing Enter
- Immediate visual feedback for menu selection
- Clean terminal control and restoration

### Menu Choice Enumerator

Menu options are defined using the `MenuChoice` enum (`main.c:39-45`):
...
This provides type-safe menu option handling and makes the code more maintainable.
...
**Usage examples:**
- `main.c:753` - `if(current->id == receipt_id)` in `update_receipt()`
- `main.c:809` - `if(current->id == receipt_id)` in `delete_receipt()`
- `main.c:849` - `if(current->id == receipt_id)` in `view_receipt()`
...
**Regeneration during load** (`main.c:477`):
```c
...
**Doubly linked list structure** (`main.c:51-57`):
```c
typedef struct Receipt {
...
```c
// Append mode for new recipes (main.c:539)
FILE *fptr = fopen(FILE_NAME, "a");

// Full rewrite only for updates/deletes (main.c:562)
uint8_t rewrite_receipts_to_file(Receipt *head){
    // Only called when necessary to minimize I/O overhead
}
```

**Alphabetical sorting** (`main.c:635-689`):
```c
